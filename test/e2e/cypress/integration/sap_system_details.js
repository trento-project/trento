...
import { selectedSystem, attachedHosts } from '../fixtures/sap-system-details/selected_system'
    ...
    describe('The system layout shows all the running instances', () => {
        ...
        selectedSystem.Hosts.forEach((instance, index) => {
            it(`should show hostname "${instance.Hostname}" with the correct values`, () => {
                cy.get('.eos-table').eq(0).find('tr').eq(index+1).find('td').as('tableCell')
                ...
                cy.get('@tableCell').eq(6).find('span').should('have.class', instance.StatusBadge)
            })
        })
        ...
        states.forEach(([state, badge]) => {
            it(`should show ${state} badge in instace when SAPControl-GRAY state is received`, () => {
                ...
                cy.get('@tableCell').eq(6).find('span').should('have.class', `badge-${badge}`)
            })
        })
    ...
    describe('The hosts table shows the attached hosts to this SAP system', () => {
        attachedHosts.forEach((host, index) => {
            it(`should show ${host.Name} with the correct link and data`, () => {
                cy.get('.eos-table').eq(1).find('tr').eq(index+1).find('td').as('tableCell')
                cy.get('@tableCell').eq(1).should('contain', host.Name)
                cy.get('@tableCell').eq(1).find('a').should('have.attr', 'href', `/hosts/${host.AgentId}`)
