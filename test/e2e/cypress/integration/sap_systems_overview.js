context('SAP Systems Overview', () => {
    ...
    describe('Registered SAP Systems should be available in the overview', () => {
        ...
        describe('Attached databases are the expected ones', () => {
            availableSAPSystems.forEach(({ sid: sid, attachedDatabase: attachedDatabase }) => {
                ...
                it(`should have a link to the attached HANA database with id: ${attachedDatabase.id}`, () => {
                    ...
                    cy.location('pathname').should('eq', `/databases/${attachedDatabase.id}`);
                    cy.go('back');
                })
            })
        })
            ...
            availableSAPSystems.forEach(({ id: id, instances: instances }) => {
                    ...
                    cy.get(`#instances-${id}`)
                        ...
                        .each((row, index) => {
                            cy.wrap(row).within(() => {
                                ...
                                cy.get('td').eq(3).should('contain', instances[index].instanceNumber);
                                cy.get('td').eq(4).should('contain', instances[index].systemReplication);
                                cy.get('td').eq(4).should('contain', instances[index].systemReplicationStatus);
                                cy.get('td').eq(5).should('contain', instances[index].clusterName);
                ...
                it(`should have a link to the hosts`, () => {
                    cy.get(`#instances-${id}`)
                        .find('tr')
                        .each((row, index) => {
                            cy.wrap(row).get('td').contains(instances[index].hostname).click({ force: true });
                            cy.location('pathname').should('eq', `/hosts/${instances[index].hostID}`);
                            cy.go('back');
                        })
                })
            })
